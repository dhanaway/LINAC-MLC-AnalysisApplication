...
#Now with MotherProgram can call this from python terminal and then subsequently run dataBaseAnalyzerWDLG to sort the results and analyze them

#Only thing is when figures are not visable (off) it gets angry and doesnt run through python - figures are turned on and pile up until the very end of this script when they are all closed
...
unwind_protect
  ...
  disp('1')
  disp(AArray)
  disp(length(sortedListinginit))
  disp('actual array is"')
  disp(sortedListinginit)
